...
#include "time.h"
/*
The level system, to keep things simple has 1 meter per tile for now, 
We set the zoom level to a value so that the level will be visible in the end monitor
*/
    ...
    for (u32 j = 0; j < level->pit_count_y; j+=1) {
      for (u32 mb_i = 0; mb_i < level->pit_count_x; mb_i+=1) {
        for (u32 mb_j = 0; mb_j < level->pit_count_x; mb_j+=1) {
          ...
          if (entity_is_pit) {
            make_pit_entity(level_add_entity(level), v2m(i * 3 + mb_i, j * 3 + mb_j), level->pit_dim_mt);
          } else {
            make_ground_entity(level_add_entity(level), v2m(i * 3 + mb_i, j * 3 + mb_j), level->pit_dim_mt);
          }
        }
      }
    }
  ...
  for (u32 i = 1; i < 9; i += 3) {
    for (u32 j = 1; j < 9; j += 3) {
      if ((rand()%10)+level_idx < 5) {
        make_human_entity(level_add_entity(level), v2m(i,j), level->pit_dim_mt);
      }else {
        make_goblin_entity(level_add_entity(level), v2m(i,j), level->pit_dim_mt);
      }
    }
  } 
...
void level_render(Level *level, Game_State *gs) {
  if (level) {
    R2D_Cmd cmd = (R2D_Cmd){ .kind = R2D_CMD_KIND_SET_CAMERA, .c = (R2D_Cam){ .offset = v2m(gs->game_viewport.w/2,gs->game_viewport.h/2), .origin = v2m(level->pit_count_x * level->pit_dim_mt * 3 / 2,level->pit_count_y * level->pit_dim_mt * 3 / 2), .zoom = gs->zoom, .rot_deg = 0} };
